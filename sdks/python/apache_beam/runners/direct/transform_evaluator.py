    ...
    if self.test_stream_event_channel:
      try:
        ...
        if event.HasField('element_event'):
          ...
          elements = [
              TimestampedValue(
                  self.test_stream.coder.decode(e.encoded_element),
                  Timestamp(micros=e.timestamp)) for e in element_event.elements
          ]
          events.append(
              ElementEvent(timestamped_values=elements, tag=element_event.tag))
        ...
        events += ([
            e
            for e in self.test_stream._tear_down(self.test_stream.output_tags)
        ])
    else:
