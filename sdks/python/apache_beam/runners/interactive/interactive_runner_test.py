...
class TuneForFlinkTest(unittest.TestCase):
  ...
  def test_create_a_new_cluster_for_a_new_pipeline(self, m_env):
    # Mock the current environment to use a non-global-singleton Clusters.
  ...
  @patch.object(DataprocClusterManager, 'cleanup', mock_delete_cluster)
  @patch('apache_beam.runners.interactive.interactive_environment.current_env')
  def test_reuse_a_cluster_for_a_known_pipeline(self, m_env, m_create_cluster):
    clusters = ib.Clusters()
    m_env().clusters = clusters
  ...
  @patch.object(DataprocClusterManager, 'cleanup', mock_delete_cluster)
  @patch('apache_beam.runners.interactive.interactive_environment.current_env')
  def test_reuse_a_known_cluster_for_unknown_pipeline(
      self, m_env, m_create_cluster):
    clusters = ib.Clusters()
    m_env().clusters = clusters
