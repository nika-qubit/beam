...
class Clusters:
  ...
  default cluster metadata to None:
  ...
  def _cleanup(self, dcm: DataprocClusterManager) -> None:
    dcm.cleanup()
    self.dataproc_cluster_managers.pop(dcm.cluster_metadata)
    self.master_urls.pop(dcm.cluster_metadata.master_url)
    for p in dcm.pipelines:
      self.pipelines.pop(p)
