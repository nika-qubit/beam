...
def _fake_a_running_test_stream_service(pipeline):
  ...
  ie.current_env().set_test_stream_service_controller(pipeline,
                                                      test_stream_service)
...
class CaptureControlTest(unittest.TestCase):
  ...
  @patch('apache_beam.testing.test_stream_service.TestStreamServiceController'
         '.stop')
  ...
  def test_capture_size_not_reached_when_file_size_under_limit(self):
    ib.options.capture_size = 100
    cache = StreamingCache(cache_dir=None)
  ...
  def test_capture_size_reached_when_file_size_above_limit(self):
    ib.options.capture_size = 1
    cache = StreamingCache(cache_dir=None)
    cache.sink(['my_label'], is_capture=True)
    cache.write([TestStreamFileRecord(
        element_event=TestStreamPayload.Event.AddElements(
            elements=[
                TestStreamPayload.TimestampedElement(
                    encoded_element=coders.FastPrimitivesCoder().encode('a'),
                    timestamp=0)]))], 'my_label')
    self.assertTrue(cache.exists('my_label'))
    ie.current_env().set_cache_manager(cache)
    self.assertTrue(
        ie.current_env().options.capture_control.is_capture_size_reached())
