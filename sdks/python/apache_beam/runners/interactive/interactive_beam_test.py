...
from apache_beam.runners.interactive.testing.mock_clusters import mock_create_cluster
from apache_beam.runners.interactive.testing.mock_clusters import mock_delete_cluster
...
class InteractiveBeamClustersTest(unittest.TestCase):
  ...
  @patch.object(DataprocClusterManager, 'cleanup', mock_delete_cluster)
  def test_create_but_reuse_a_known_cluster(self, m_create_cluster):
    ...
    m_create_cluster.assert_not_called()
  ...
  @patch.object(
      DataprocClusterManager, 'create_flink_cluster', mock_create_cluster)
  @patch.object(DataprocClusterManager, 'cleanup')
  def test_cleanup_by_a_pipeline(self, m_delete_cluster):
    meta = ClusterMetadata(project_id='test-project')
    ...
    # Delete the cluster.
    m_delete_cluster.assert_called_once()
  ...
  @patch.object(
      DataprocClusterManager, 'create_flink_cluster', mock_create_cluster)
  @patch.object(DataprocClusterManager, 'cleanup')
  def test_not_cleanup_if_multiple_pipelines_share_a_manager(
      self, m_delete_cluster):
    meta = ClusterMetadata(project_id='test-project')
    dcm = self.clusters.create(meta)
  ...
  @patch.object(
      DataprocClusterManager, 'create_flink_cluster', mock_create_cluster)
  @patch.object(DataprocClusterManager, 'cleanup')
  def test_cleanup_by_a_master_url(self, m_delete_cluster):
    meta = ClusterMetadata(project_id='test-project')
    _ = self.clusters.create(meta)
  ...
  @patch.object(
      DataprocClusterManager, 'create_flink_cluster', mock_create_cluster)
  @patch.object(DataprocClusterManager, 'cleanup')
  def test_cleanup_by_meta(self, m_delete_cluster):
    known_meta = ClusterMetadata(
        project_id='test-project', region='test-region')
  ...
  @patch.object(
      DataprocClusterManager, 'create_flink_cluster', mock_create_cluster)
  @patch.object(DataprocClusterManager, 'cleanup')
  def test_cleanup_everything(self, m_delete_cluster):
    meta = ClusterMetadata(project_id='test-project')
    ...
    self.assertEqual(m_delete_cluster.call_count, 2)
    self.assertNotIn(meta, self.clusters.dataproc_cluster_managers)
  ...
  @patch.object(
      DataprocClusterManager, 'create_flink_cluster', mock_create_cluster)
  @patch.object(DataprocClusterManager, 'cleanup')
  def test_cleanup_noop_unknown_cluster(self, m_delete_cluster):
    meta = ClusterMetadata(project_id='test-project')
    dcm = self.clusters.create(meta)
    p = beam.Pipeline()
    self.clusters.pipelines[p] = dcm
